...
//variables to change stuff
	...
	var clickDistance = 17; //max distance from center point of peices which will 'click' on the peice
...
function availability(){
	...
	if(beginingPlacement){
		...
		for(i in roads){
			if(players[turn].roads < 15 && players[turn].brick >= 1 && players[turn].wood >= 1){
				if(roads[i].ownership == -2){
					roads[i].ownership = -1;
				}else{}
			}else{
	...
	//updates side display values
	document.querySelector('#p' + turn + ' > tbody > tr > .wood').innerHTML = players[turn].wood;
	document.querySelector('#p' + turn + ' > tbody > tr > .wool').innerHTML = players[turn].wool;
	document.querySelector('#p' + turn + ' > tbody > tr > .grain').innerHTML = players[turn].grain;
	document.querySelector('#p' + turn + ' > tbody > tr > .ore').innerHTML = players[turn].ore;
	document.querySelector('#p' + turn + ' > tbody > tr > .brick').innerHTML = players[turn].brick;
}
...
function create_vertex(x,y, vertexNumber, last) {
	...
	var ownership = 'none';
	var port = 'none';
	eval('vertices.v' + x + '_' + y + '= {x,y,rollNumber,rollDots,type,pieceStatus,ownership,port}')
