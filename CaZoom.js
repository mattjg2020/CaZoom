...
function updateResourceDisplay(){
	...
	for(i in players){
		...
		if(players[i].awards.longestRoad){
			awardsTD.innerHTML += 'Longest Road<br>'
		}else{}

		if(players[i].awards.largestArmy){
			awardsTD.innerHTML += 'Largest Army<br>'
		}else{}
...
function mouseClicked(){
	if(document.getElementById('pageDarken').style.visibility == 'visible'){
	}else{
		var mouseD;
		var centerX;
		var centerY;
...
function updateTradePage(clearNum){
	...
	for(i in hexagonTypes){
		if(tradee.value == hexagonTypes[i]){
			traderType.selectedIndex = i;
		}else{}
	}
