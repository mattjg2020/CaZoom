//Variables of utility
	...
	var robber = 'https://mattjg2020.github.io/robber.png';
	var devCard_icon = 'https://mattjg2020.github.io/devCard_icon.png';
	var names = [];
	var players = [];
	var turn = -1;
...
function updateResourceDisplay(){
	...
	for(i in players){
		...
		if(players[i].awards.longestRoad){
			awardsTD.innerHTML += 'Longest Road'
		}else{}

		if(players[i].awards.largestArmy){
			awardsTD.innerHTML += 'Largest Army'
		}else{}
...
function checkLongestRoad(){
	...
	for(r in roads){
		if(roads[r].ownership == turn){
			...
			for(a in adjacentRoads){
				roadTreeLayer1.push({'road': adjacentRoads[a], 'parent': r})
			}
		}else{}
	}	
	...
	for(n = 1; n < 15; n++){
		...
		for(i in currentLayer){
			...
			for(j in allAdjacentRoads){
				//check for the good roads
				for(k in parentAdjacentRoads){
					if(allAdjacentRoads[j] != parentAdjacentRoads[k] && allAdjacentRoads[j] != currentLayer[i].parent){
						//if the road isn't adjacent to the parent and isn't the parent then it is good
						//this prevents backtracking
						nextLayer.push({'road': allAdjacentRoads[j], 'parent': currentLayer[i].road})
					}else{}
				}
			}
	...
	if(longestRoadForPlayer > longestRoad){
		...
		undoList[undoList.length - 1].push('updateResourceDisplay()');
...
function buildingBought(i){
	if(beginingPlacement && !placedSettlement){
		...
		undoFunctions.push('updateResourceDisplay()')
